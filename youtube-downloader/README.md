# 📹 High Quality YouTube Downloader

A powerful Python tool that downloads YouTube videos in the highest possible quality (1080p, 4K) using `yt-dlp`.

## 🚀 Features
* Downloads 1080p, 4K, 8K videos with audio.
* Merges video and audio automatically using FFmpeg.
* Simple and fast.

## 🛠️ Installation

1. Install the requirement:
   ```bash
   pip install -r requirements.txt

2-Crucial Step (FFmpeg Setup):

This tool requires FFmpeg to merge video and audio.

Download the correct version directly from [here](https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip).. ->
Extract the zip file, find bin/ffmpeg.exe, and place it inside this project folder (next to main.py).

▶️ Usage
Run the script in your terminal:

python main.py

⚠️ Disclaimer
This tool is created for educational purposes only. The user assumes all responsibility for adhering to YouTube's Terms of Service and copyright laws.
